# Q-CRAFT (2024-11-15) graph-oracle parity validation
...
The test reads every `compute_*` formula attribute from one `Model.from_defaults`
per scenario and compares those values to FormulaEvaluator. It does not drive
Microsoft Excel. Callers still use `compute_*`.
