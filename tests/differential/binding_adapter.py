...
def model_input_overrides(
    data: object,
    *,
    inputs: Mapping[str, object],
    input_series: Sequence[Mapping[str, Any]],
) -> dict[str, object]:
    """Keyword overrides for one ``Model.from_defaults`` call.

    Scalar series take the scenario value. Keyed series with no records stay
    on ``data.*_DEFAULT``. Keyed series with records are that default with
    those cells replaced.
    """
    overrides: dict[str, object] = {}
    for series in input_series:
        name = str(series["id"])
        if series["key_fields"]:
            records = _record_sequence(inputs.get(name))
            if not records:
                continue
            ...
            if not hasattr(data, attr):
                ...
                raise TypeError(
                    f"Model.from_defaults override {name!r} needs {module_name}.{attr}"
                )
            overrides[name] = overlay_series_values(
                series, getattr(data, attr), records
            )
            continue
        if name not in inputs:
            raise TypeError(f"scenario inputs are missing scalar series {name!r}")
        overrides[name] = inputs[name]
    return overrides
